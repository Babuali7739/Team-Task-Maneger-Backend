...
export const assignSubTodo = async (req, res) => {
  try {
    const { subTodoId } = req.params;
    ...
    // 🔐 SAME PERMISSION LOGIC AS YOUR FILE
    const isTodoOwner = todo.createdBy.toString() === userId;
    ...
    // ❌ Read users can't assign
    if (!isTodoOwner && !hasWritePermission) {
      return res.status(403).json({
        success: false,
        message: "Permission denied: you can't assign users",
      });
    }

    // ✅ Assign user
    subTodo.assignedTo = assignedTo || null;

    await subTodo.save();

    // ✅ populate assigned user
    await subTodo.populate("assignedTo", "_id userName email");
